...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["accion"])) {
    ...
    $redireccion = "tutoria_unidades.php?unidad=" . $unidad . "&semana=" . $semana;
    ...
    if (!$unidadValida || !$semanaValida) {
        ...
        header("Location: tutoria_unidades.php");
        exit;
    ...
    if ($accion === "guardar_semana") {
        ...
        try {
            ...
            $archivoInfo = materiales_guardar_archivo(
                $_FILES["archivo"] ?? [],
                MATERIALES_DIRECTORIO_TUTORIA_UNIDADES,
                "backend/uploads/materiales/tutoria/unidades/"
            );

            $existente = $conexion->prepare("
                SELECT id_material, ruta_archivo FROM profesor_tutoria_unidad_archivos
            ...
            if ($existente) {
                ...
                $conexion->prepare("
                    UPDATE profesor_tutoria_unidad_archivos
                ...
                $conexion->prepare("
                    INSERT INTO profesor_tutoria_unidad_archivos
        ...
        if (!$existente) {
            ...
            $conexion->prepare("
                DELETE FROM profesor_tutoria_unidad_archivos
                WHERE id_material = ? AND id_profesor = ? AND id_grado_seccion = ? AND id_periodo = ?
...
if ($unidadSel && $semanaSel) {
    ...
    $stmt = $conexion->prepare("
        SELECT * FROM profesor_tutoria_unidad_archivos
        WHERE id_profesor = ? AND id_grado_seccion = ? AND id_periodo = ? AND unidad = ? AND semana = ?
    ");
...
if ($unidadSel) {
    $stmt = $conexion->prepare("
        SELECT unidad, semana FROM profesor_tutoria_unidad_archivos
...
<head>
    ...
    <title>Unidades · Tutoría · Panel del Profesor - I.E.P. 88044 Abraham Valdelomar</title>
...
<header>
    <div>
        ...
        <h1><?= icon("layers") ?> Unidades · Tutoría</h1>
    </div>
...
<main>
    ...
    <div class="context-bar">
        <nav class="breadcrumbs">
            ...
            <span>›</span>
            <a href="tutoria_unidades.php">Unidades</a>
            <?php if ($unidadSel): ?>
                <span>›</span>
                <?php if ($semanaSel): ?>
                    <a href="tutoria_unidades.php?unidad=<?= $unidadSel ?>">Unidad <?= $unidadSel ?></a>
                    <span>›</span>
        ...
        <div class="cards">
            <?php for ($u = 1; $u <= $totalUnidades; $u++): ?>
                <div class="card">
                    ...
                    <a href="tutoria_unidades.php?unidad=<?= $u ?>">Abrir unidad</a>
                </div>
            <?php endfor; ?>
        </div>
        ...
        <div class="cards">
            <?php for ($s = 1; $s <= materiales_total_semanas($tutoria["nivel"]); $s++): ?>
                <div class="card">
                    ...
                    <a href="tutoria_unidades.php?unidad=<?= $unidadSel ?>&semana=<?= $s ?>">Abrir semana</a>
                </div>
            <?php endfor; ?>
        </div>
                ...
                <?php if ($archivoSemana): ?>
                    ...
                    <div class="row-actions">
                        <a class="btn-mini" target="_blank" rel="noopener"
                           href="../backend/materiales/archivo.php?modulo=tutoria_unidad&id=<?= (int) $archivoSemana["id_material"] ?>&accion=ver">
                            Ver
                        </a>
                        <a class="btn-mini" href="../backend/materiales/archivo.php?modulo=tutoria_unidad&id=<?= (int) $archivoSemana["id_material"] ?>&accion=descargar">
                            Descargar
                        </a>
