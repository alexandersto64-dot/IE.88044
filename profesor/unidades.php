...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["accion"])) {
    ...
    $redireccion = "unidades.php?unidad=" . $unidad . "&semana=" . $semana;
    ...
    if ($accion === "guardar_curso") {
        ...
        try {
            ...
            if ($existente) {
                ...
                flash_set("Archivo de " . materiales_curso_nombre($curso) . " reemplazado correctamente.", "success");
                ...
                flash_set("Archivo de " . materiales_curso_nombre($curso) . " subido correctamente.", "success");
        ...
        if (!$existente) {
            ...
            $conexion->prepare("
                DELETE FROM profesor_unidad_archivos WHERE id_material = ? AND id_profesor = ?
            ")->execute([$existente["id_material"], $idProfesor]);

            materiales_eliminar_archivo_fisico($existente["ruta_archivo"]);

            flash_set("Archivo de " . materiales_curso_nombre($curso) . " eliminado correctamente.", "success");
...
<head>
    ...
    <title>Unidades · Panel del Profesor - I.E.P. 88044 Abraham Valdelomar</title>
...
<main>
    ...
    <nav class="breadcrumbs">
        ...
        <?php if ($unidadSel): ?>
            <span>›</span>
            <?php if ($semanaSel): ?>
                <a href="unidades.php?unidad=<?= $unidadSel ?>">Unidad <?= $unidadSel ?></a>
                <span>›</span>
        ...
        <div class="cards">
            <?php for ($u = 1; $u <= MATERIALES_TOTAL_UNIDADES; $u++): ?>
                <div class="card">
                    ...
                    <a href="unidades.php?unidad=<?= $u ?>">Abrir unidad</a>
                </div>
            <?php endfor; ?>
        </div>
        ...
        <div class="cards">
            <?php for ($s = 1; $s <= MATERIALES_TOTAL_SEMANAS; $s++): ?>
                <div class="card">
                    ...
                    <a href="unidades.php?unidad=<?= $unidadSel ?>&semana=<?= $s ?>">Abrir semana</a>
                </div>
            <?php endfor; ?>
        </div>
